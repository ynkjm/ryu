...
from ryu.ofproto.ofproto_v1_0 import NX_ROLE_MASTER, NX_ROLE_SLAVE
...
class MultipleControllers(object):
    ...
    def __init__(self, role, zhost):
        ...
        self.role = NX_ROLE_MASTER if role == 'master' else \
            NX_ROLE_SLAVE
        self.zhost = zhost
        self.dpset = {}
        self.cnodes = {}
    ...
    def role_request(self, dp, role):
        msg = dp.ofproto_parser.NXTRoleRequest(dp, role)
        dp.send_msg(msg)
    ...
    def init_zookeeper(self):
        ...
        if self.role == NX_ROLE_MASTER:
            self.cid = self.create_znode(MASTER_PATH, '',
                                         [ZOO_OPEN_ACL_UNSAFE],
                                         zookeeper.EPHEMERAL)
            if self.cid == None:
                LOG.error('master already exists', self.role)
                raise
        elif self.role == NX_ROLE_SLAVE:
            self.cid = self.create_znode(SLAVE_PATH, '',
                                         [ZOO_OPEN_ACL_UNSAFE],
                                         zookeeper.EPHEMERAL |
                                         zookeeper.SEQUENCE)
    ...
    def handle_datapath(self, ev):
        self.dpset[ev.dp.id] = ev.dp
        self.role_request(ev.dp, self.role)
